...
from ui_profile import Ui_Form
...
class ProfilePage(QWidget, Ui_Form):
    ...
    def set_photo(self):
        ...
        if name:
            ...
            image.save('Изображения/user_image.png')
            self.btn_image.setText('')
            # Установка фото на кнопку
            self.btn_image.setStyleSheet(
                f"border-image: url('Изображения/user_image.png')")
            db_help.set_picture(self.connection,
                                self.person_id)  # Обновление фото в самое БД
