...
class RatingPage(QWidget, Ui_Form):
    ...
    def refresh_rating(self):
        ...
        for i, row in enumerate(sorted(rating, key=lambda x: -x[2])):
            ...
            with open('Изображения/rating_image.png', mode='wb') as g:
                # Проверка на наличие фото профиля у пользователя
                if picture:
                    # Запись потока байт в файл
                    g.write(picture)
                    # Создание картинки из файла
                    picture = QPixmap('Изображения/rating_image.png')
                    picture = picture.scaled(QSize(50, 50))  # Смена размеров
                    image.setPixmap(picture)  # Установка картинки в label

            self.rating_table.setCellWidget(i, 0,
                                            image)  # Добавление label в таблицу
